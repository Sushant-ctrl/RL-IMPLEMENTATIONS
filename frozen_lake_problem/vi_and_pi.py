...
def policy_evaluation(P, nS, nA, policy, gamma=0.9, tol=1e-3):
    ...
    while(True):
        ...
        for i in range(nS):
            sum = 0
            for j in range(len(P[i][policy[i]])):
                sum = P[i][policy[i]][j][2] + P[i][policy[i]][j][0]*value_function[P[i][policy[i]][j][1]]*gamma + sum
    ...
    for i in range(nS):
        for j in range(nA):
            ...
            for k in range(len(P[i][j])):
                sum = sum + P[i][j][k][0]*P[i][j][k][2] + P[i][j][k][0]*value_from_policy[P[i][j][k][1]]*gamma
            s[j]=sum
        new_policy[i] = np.argmax(s)
    ############################
    return new_policy
    ...
    while True:
        ...
        for i in range(nS):
            for j in range(nA):
                summ = 0
                for k in range(len(P[i][j])):
                    summ = summ + P[i][j][k][0]*P[i][j][k][2] + P[i][j][k][0]*value_function[P[i][j][k][1]]*gamma
                s[j] = summ
            policy[i] = np.argmax(s)
            value_function[i] = s[policy[i]]
        if np.max(np.abs(value_function - prev_value_function)) < tol:
            break
        else:
            prev_value_function = np.copy(value_function)
